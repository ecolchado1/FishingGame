...
public class GameServerUDP extends GameConnectionServer<UUID> {
	...
	public void processPacket(Object o, InetAddress senderIP, int senderPort) {
		...
		if(messageTokens.length > 0) {	
			...
			if (messageTokens[0].compareTo("rotate") == 0) {
				...
				float axisX = Float.parseFloat(messageTokens[3]);
				float axisY = Float.parseFloat(messageTokens[4]);
				float axisZ = Float.parseFloat(messageTokens[5]);

				String[] pos = {messageTokens[2], messageTokens[3], messageTokens[4], messageTokens[5]};
				sendRotationMessages(clientID, angle, axisX, axisY, axisZ);
			}

			if (messageTokens[0].compareTo("addScore") == 0) {
				UUID clientID = UUID.fromString(messageTokens[1]);
				int amount = Integer.parseInt(messageTokens[2]);
				playerScores.put(clientID, playerScores.getOrDefault(clientID, 0) + amount);

				System.out.println("Updating " + clientID + " score");
				sendAddScoreMessage(clientID, amount);
			}

			// Message to request all other players scores
			if (messageTokens[0].compareTo("getOtherScores") == 0) {
				UUID clientID = UUID.fromString(messageTokens[1]);
				sendOtherScoresMessage(clientID);
			}

			if (messageTokens[0].equals("isSinglePlayer")) {
				UUID clientID = UUID.fromString(messageTokens[1]);

				// If only one client is connected, it's single-player
				boolean isSinglePlayer = (getClients().size() <= 1);
				
				try {
					sendPacket("isSinglePlayer," + isSinglePlayer, clientID);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	public void sendOtherScoresMessage(UUID clientID) {
		StringBuilder msg = new StringBuilder("otherScores");

		for (Map.Entry<UUID, Integer> entry : playerScores.entrySet()) {
			UUID id = entry.getKey();
			if (!id.equals(clientID)) {
				msg.append(",").append(id.toString()).append(":").append(entry.getValue());
			}
		}
		try { sendPacket(msg.toString(), clientID); } 
		catch (IOException e) { e.printStackTrace(); }
	}

	public void sendAddScoreMessage(UUID clientID, int amount) {
		try {
			String msg = "scoreAdded," + amount;
	...
	// Informs a client that a remote client�s avatar has changed rotation
	public void sendRotationMessages(UUID clientID, float angle, float axisX, float axisY, float axisZ) {
		try {
